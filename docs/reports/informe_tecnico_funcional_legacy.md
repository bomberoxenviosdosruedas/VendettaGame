# Informe Técnico Funcional: Proyecto Vendetta (Legacy)

Este documento detalla la lógica de negocio, arquitectura y datos del proyecto "Vendetta", un juego de estrategia basado en navegador (mafia theme), extraído mediante ingeniería inversa del código fuente legacy (Zend Framework 1.x / PHP 5).

> **Nota:** Este informe se centra exclusivamente en la modalidad "Vendetta" (Mafia), ignorando la lógica del modo "Space4k".

## 1. Arquitectura y Estructura

*   **Framework:** Zend Framework 1.x (MVC).
*   **Entrada:** `index.php` redirige a `public/index.php`, que inicia `Zend_Application`.
*   **Módulos:**
    *   `default` (Login/Landing)
    *   `mob` (Lógica principal del juego)
    *   `register` (Registro)
    *   `admintool` (Panel de administración)
*   **Modelos (`application/models` y `library/Mob/Model`):**
    *   `Edificio`: Representa una ciudad/base. Maneja recursos y coordenadas.
    *   `Habitacion`: Edificios construibles dentro de una base (Oficina, Armería, etc.).
    *   `Tropa`: Unidades militares (Matón, Portero, etc.).
    *   `Entrenamiento`: Investigaciones/Tecnologías.
    *   `Misiones`: Lógica de flotas/envíos (Atacar, Transportar, Ocupar).
    *   `Batallas`: Motor de combate y reportes.

## 2. Base de Datos y Configuración

Se adjunta el esquema de base de datos recuperado y la configuración principal del juego.

### 2.1. Configuración del Juego (`gameconfig.php`)
...
### 2.2. Esquema de Base de Datos (`vendetta_plus_old.sql`)
...
## 3. Mecánicas Nucleares (Critical Path)

### 3.1. Recursos y Producción

Los edificios de producción generan recursos basados en su nivel.

**Fórmulas Generales de Producción (`mob_habitaciones_manager.php`):**

*   **Armas (Armería):** `Base(10) + 10 * ((Nivel + 150) / 12)^3`
*   **Munición (Almacén de Munición):** `Base(10) + 10 * ((Nivel + 150) / 12)^3`
*   **Alcohol (Cervecería):** `Base(10) + 50 * ((Nivel + 102) / 2)^2`
*   **Dólares:** Se generan mediante la venta de alcohol en Taberna o Contrabando.
    *   **Consumo Taberna:** `(ProducciónTaberna * 7) + 3` unidades de alcohol.
    *   **Consumo Contrabando:** `(ProducciónContrabando * 4) + 1` unidades de alcohol.
    *   **Lógica:** Si hay suficiente alcohol producido (Cervecería), la Taberna y el Contrabando producen dólares al 100%. Si no, producen proporcionalmente al alcohol disponible.

### 3.2. Edificios (Habitaciones)

Las "habitaciones" son las estructuras construibles dentro de un edificio base.

**Fórmula de Costo:**
```math
Costo = CostoBase * (Nivel + 1)^2
```
Los costos base (`_arm`, `_mun`, `_dol`) están definidos por clase.

**Fórmula de Tiempo:**
```math
Segundos = ( (Nivel^2) * DuracionBase ) / NivelOficina
```
*(Nota: Se aplica una reducción de `round($segundos / 3)` en `Mob_Habitacion_Abstract::getTiempoMejora`)*.

**Lista de Edificios (Vendetta):**

| Nombre (Clase) | Costos Base (Arm/Mun/Dol) | Duración Base | Requisitos |
| :--- | :--- | :--- | :--- |
| **Oficina** (`Oficina`) | 100 / 200 / 0 | 900 | - |
| **Escuela** (`Escuela`) | 1000 / 1000 / 25 | 2000 | - |
| **Armería** (`Armeria`) | 12 / 60 / 0 | 500 | - |
| **Munición** (`Municion`) | 9 / 15 / 0 | 600 | - |
| **Cervecería** (`Cerveceria`) | 20 / 20 / 0 | 1000 | - |
| **Taberna** (`Taberna`) | 10 / 50 / 0 | 1500 | Cervecería (1) |
| **Contrabando** (`Contrabando`) | 2000 / 5000 / 500 | 4000 | Oficina (5), Cervecería (8) |
| **Almacén Armas** (`AlmacenArm`) | 100 / 500 / 0 | 9000 | Armería (5) |
| **Depósito Mun.** (`Deposito`) | 500 / 600 / 0 | 12000 | Munición (5) |
| **Almacén Alc.** (`AlmacenAlc`) | 200 / 200 / 0 | 8000 | Cervecería (5) |
| **Caja Fuerte** (`Caja`) | 2000 / 2000 / 1000 | 16000 | Taberna (5) |
| **Campo Entreno** (`Campo`) | 1000 / 2500 / 0 | 5600 | - |
| **Seguridad** (`Seguridad`) | 0 / 0 / 0 | 6000 | Campo (2) |
| **Torreta** (`Torreta`) | 1000 / 2000 / 200 | 4500 | Oficina (5) |
| **Minas** (`Minas`) | 2000 / 2000 / 150 | 3000 | Oficina (5) |

## 4. Militar y Combate

### 4.1. Unidades (Tropas)

Las unidades se dividen en Ataque y Defensa (Seguridad).

**Fórmula de Tiempo de Entrenamiento:**
```math
Segundos = DuracionBase / NivelCampoEntrenamiento
```

**Stats Principales (Ejemplos):**

*   **Matón (`Maton`):** Atq: 5, Def: 5, Cap: 200, Vel: 1600.
*   **Portero (`Portero`):** Atq: 8, Def: 6, Cap: 400, Vel: 2000. Req: Extorsión.
*   **Espía (`Espia`):** Atq: 1, Def: 1, Vel: 400,000 (Muy rápido). Misión: Espionaje.
*   **Mercenario (`Mercenario`):** Atq: 1000, Def: 1200 (Elite). Costo alto.

### 4.2. Motor de Combate (`Mob_Combat_Abstract`)

El sistema de batalla se resuelve en 5 rondas máximas.

**Lógica de Ronda:**
1.  **Cálculo de Poder Total:** Suma de Ataque (PAA) y Defensa (PDA) del Atacante vs Defensor (PAD, PDD).
2.  **Bonificaciones:** Se aplican multiplicadores si la unidad tiene ventaja sobre otra (definido en `_modificadores`).
3.  **Cálculo de Bajas (`scrap_calc`):**
    *   Usa una fórmula logarítmica compleja basada en la relación de fuerzas `log(Atq / Def)`.
    *   Determina el porcentaje de pérdidas para cada bando (`PPA` = % Pérdidas Atacante, `PPD` = % Pérdidas Defensor).
4.  **Distribución de Daño:** Las bajas se aplican proporcionalmente a todas las unidades involucradas.
5.  **Fin de Ronda:** Si un bando se queda sin unidades, termina el combate.

**Robo de Recursos:**
*   Se calcula la capacidad de carga total de las naves supervivientes del atacante.
*   Se restan los recursos protegidos por almacenes (Depósito/Caja).
*   Se roba hasta llenar la capacidad, priorizando proporcionalmente lo disponible.

## 5. Mapa y Tecnologías

### 5.1. Mapa y Distancias
*   **Coordenadas:** `[Ciudad]:[Barrio]:[Edificio]` (Ej: 1:1:5).
*   **Fórmula Distancia:**
    ```php
    sqrt( (dy * 15)^2 + (dx * 17)^2 ) * 1000
    ```
    *Donde dx y dy son diferencias calculadas con una función modular específica para la grilla isométrica.*

### 5.2. Tecnologías (Entrenamientos)

Investigaciones que desbloquean unidades o mejoran stats.

**Arbol Tecnológico (Extracto):**
*   **Rutas:** Desbloquea velocidad y mapas.
*   **Encargos:** Prerrequisito para Explosivos.
*   **Extorsión:** Requisito para Portero y Acuchillador.
*   **Administración:** Mejora gestión de base.
*   **Combate, Tiro, Guerrilla:** Mejoran stats de unidades y desbloquean unidades de élite (Mercenario, Ninja).

**Bonificaciones:**
*   Nivel de "Espionaje" determina el detalle del reporte de espionaje.
*   Nivel de "Honor" afecta el cálculo de puntos de batalla.
