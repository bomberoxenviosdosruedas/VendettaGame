-- 1. View-Model RPC for Dashboard
-- Returns aggregated data for the dashboard in a single round-trip.
-- This function materializes resources first to ensure fresh data.
...
-- 2. RPC para cancelar construcción
CREATE OR REPLACE FUNCTION public.cancelar_construccion(p_cola_id uuid)
RETURNS json AS $$
DECLARE
    v_cola record;
BEGIN
    SELECT * INTO v_cola FROM public.cola_construccion WHERE id = p_cola_id;
    IF v_cola IS NULL THEN
        RETURN json_build_object('error', 'Item no encontrado');
    END IF;

    -- Verificar propiedad (JOIN propiedad)
    IF NOT EXISTS (SELECT 1 FROM public.propiedad WHERE id = v_cola.propiedad_id AND usuario_id = auth.uid()) THEN
        RETURN json_build_object('error', 'No autorizado');
    END IF;

    DELETE FROM public.cola_construccion WHERE id = p_cola_id;
